...
struct matrix : public matrix_expression<matrix<T, F, A, tmp>> {
    ...
    // constructor
    matrix() {}
    explicit matrix(size_type size) { m_matrix.resize(size, 1); }
    ...
    const matrix &operator=(matrix<T, F, A, true> &m) {
        m_matrix.swap(m.m_matrix);
        return *this;
    }
    const matrix &operator=(value_type v) {
        this->resize(1);
        (*this)[0] = v;
        return *this;
    }
    ...
    struct matrix_type {
        ...
        struct iterator_base : public default_matrix_iterator<value_type> {
            ...
            bool operator!=(const I1 &i) const {
                assert(m_e == i.m_e);
                return m_row != i.m_row || m_col != i.m_col;
            }
            template <typename I1>
...
struct bmp_order {
    ...
    struct matrix_type {
        ...
        struct iterator_base : public default_matrix_iterator<value_type> {
            ...
            iterator_base(M *e, size_type row, size_type col) : m_row(row), m_col(col), m_e(e) {
                m_i = m_e->data().begin();
                m_i += (m_e->size(1) - row - 1) * m_e->size(2) + col;
            }
            const_reference operator*() const { return *m_i; }
            iterator_base operator++(void) {
                ++m_row;
